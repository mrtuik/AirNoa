...
- **Centre-first:** a slide with a single heading/text (or the empty state) gets `.board-body.solo` = centred horizontally + vertically, bigger text. When `show()` has a title plus content, the title appears ALONE as a centred card, then auto-advances (2s) to the content slide (unless the user navigated).
