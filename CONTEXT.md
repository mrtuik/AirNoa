...
- Teacher roasts on her own while teaching (quick, specific, affectionate; at most once every few turns; never on sensitive stuff; stops if user is low), and on "roast me" goes harder. Roasts are SPOKEN ONLY — never shown on the board (the `tags`/`bars` blocks still exist but Noa is told not to use them for roasts). Prompt lives in `persona/personas.js` (ROAST STYLE) + `TEACHER_EXTRA` in AirC.html.
- `roastMemory()` in AirC.html feeds the live system instruction (teacher only) with the user's recent lines from the last 8 sessions + `roast_notes` (quiz results saved by `addRoastNote()` in config, capped at 30). Stays local; only goes to Gemini inside the system prompt.
...
- Board heading is 13px. Board labels/headings/node names must be simple English terms (Process, Function, Location…), whatever language Noa speaks — set in `buildLiveSystemInstruction()` + `BOARD_TOOL` description.
