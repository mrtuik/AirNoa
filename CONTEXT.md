...
## Update 12 — prompt restructure: diagrams weren't automatic, topic drift on new questions
- **Root cause, not a model-quality issue:** `buildLiveSystemInstruction()` was one ~2.5k-word paragraph mixing board mechanics, personality, roast rules, memory and language into an undifferentiated block. "Always diagram a process" and "answer the newest message first" were single sentences buried inside it, so Gemini followed them inconsistently — diagrams only appeared when explicitly asked, sometimes as a bare figure with no explanation next to it, and a rephrased/new question sometimes got answered as if it were the previous topic.
- **Fix (`iota.html`, `buildLiveSystemInstruction()`):** rewritten with two **PRIORITY RULES** at the very top, above the persona and everything else: (1) LISTEN FIRST — newest user message always wins, a new question drops the old topic immediately; (2) BOARD + DIAGRAM FIRST — diagram calls are automatic/unprompted, and the diagram block must be in the **same** `show_on_board` call as the point it illustrates (not a separate bare figure). The rest of the prompt is now split into labelled sections (CONVERSATION STYLE, WHITEBOARD MECHANICS, HOW TO EXPLAIN, BOARD CONTENT RULES, VISUAL-FIRST TEACHING) instead of one paragraph, so each rule is easier for the model to hold onto.
- **`voice/live.js` `BOARD_TOOL.description`:** tightened to match — explicitly says diagrams are called automatically (never wait to be asked) and that a diagram + its point must be one tool call, not two.
- **`brain/brain.js` (typed chat, home screen) had NO board/diagram tool at all** and no LISTEN FIRST rule — it's plain `generateContent`, no function calling, so diagrams can never appear there regardless of prompt wording (by design: the whiteboard only exists on the voice/assistant screen). Added the same LISTEN FIRST priority rule to `buildSystemPrompt()` there so typed chat also answers the newest question instead of drifting to an earlier related topic.
- **Not done / next step if drift or missing diagrams persist:** the tool call is still `AUTO` mode (model decides if/when to call `show_on_board`) — no `toolConfig.functionCallingConfig` override was added. If diagrams are still inconsistent after this, consider forcing `mode: "ANY"` for turns following an explanation-type user message, or giving typed chat (`brain.js`) its own board wiring via Gemini function calling (currently text-only, no tools).
