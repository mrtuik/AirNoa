...
- `#model-container` = top **stage**, height `--stage-h` (`clamp(300px,46dvh,470px)`; `.board-open` → `clamp(230px,32dvh,340px)`). Video is `object-fit:cover; object-position:50% 0`.
- Noa dissolves: `.airc-model-wrapper` has a `mask-image` gradient (opaque to 50%, transparent at 100%) + `.stage-blur` (backdrop-filter blur ramping in, fading to white). Screen bg is pure white so it blends with the clip's white backdrop.
...
- Captions (`#captions`) sit at `--stage-h * .52` over Noa's collar/shirt; tune that multiplier if the clip framing changes.
...
- **Not yet wired to Noa**: nothing calls `NoaBoard.show` automatically. Planned: declare a Live function tool (e.g. `show_on_board`) in the `setup` message in `voice/live.js`, handle `toolCall` in `_handleServerMessage`, call `NoaBoard.show(args)`, reply with `toolResponse`, and tell the persona prompt to use it when explaining things.
