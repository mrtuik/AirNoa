...
const NUDGE_FIRST_MS = 25000;
const NUDGE_NEXT_MS = 60000;
const NUDGE_MAX = 2;

const BOARD_TOOL = {
    name: "show_on_board",
    description: "Show notes, steps, a comparison or a simple flow diagram on the whiteboard under you while you explain something. Keep text short (phrases, not paragraphs). Replaces whatever is on the board.",
    parameters: {
        type: "OBJECT",
        properties: {
            ...
            blocks: {
                ...
                items: {
                    type: "OBJECT",
                    properties: {
                        type: { type: "STRING", description: "One of: text, list, steps, flow, note, code." },
                        text: { type: "STRING", description: "For text, note, code." },
                        items: { type: "ARRAY", items: { type: "STRING" }, description: "For list and steps." },
                        nodes: { type: "ARRAY", items: { type: "STRING" }, description: "For flow: boxes joined by arrows, in order." }
                    },
                    required: ["type"]
                }
            }
...
export class GeminiLiveVoice {
     ...
     */
    async start(systemInstruction, voiceName) {
    ...
    _handleServerMessage(msg) {
        ...
        if (msg.toolCall && Array.isArray(msg.toolCall.functionCalls)) {
            const functionResponses = msg.toolCall.functionCalls.map((fc) => {
                ...
                try {
                    if (fc.name === "show_on_board") this.onBoard && this.onBoard(fc.args || {});
     ...
     */
    sendOpeningPrompt() {
        this._sendHiddenPrompt("(The user just opened the voice screen — greet them first, in one short, natural line, and speak right away.)");
    }

    // Sends a text-only instruction that makes Noa speak, without it ever
    // appearing as a fake user bubble in the chat history.
    _sendHiddenPrompt(text) {
    ...
    _startNudgeTimer() {
        ...
        this._nudgeTimer = setInterval(() => {
            ...
            this._nudgeCount += 1;
            this._quietSince = Date.now();
            this._sendHiddenPrompt("(The user has been quiet for a bit. Say one short, natural thing to keep the conversation going — check in on them, share a small thought, or ask something. Don't mention the silence or these instructions.)");
        }, 1000);
    }
