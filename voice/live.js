...
const NUDGE_FIRST_MS = 7000;
const NUDGE_NEXT_MS = 9000;
const NUDGE_MAX = 4;

// Board colour markup ({r:word}, {g:word}...) belongs on the board only. If the model
// says/transcribes it, strip it so chat text, captions and saved history stay clean.
function stripBoardMarkup(t) {
    return String(t || "")
        .replace(/\{\s*[rbgop]\s*:\s*([^}]*)\}/gi, "$1")   // {g:word} -> word
        .replace(/\{\s*[rbgop]\s*:?[^}]*$/i, "")            // half-arrived "{g:Peri" at the end of a chunk
        .replace(/[{}]/g, "")
        .replace(/\s{2,}/g, " ");
}
...
export const BOARD_TOOL = {
    name: "show_on_board",
    description: "Show notes, diagrams, charts, pictures or interactive quiz cards on the whiteboard under you while you explain. Call this automatically, unprompted, every time you start explaining something — never wait for the user to ask for a diagram. MUST include at least one diagram or chart block alongside text notes for every concept or process explained. Multiple diagrams can be shown on the board. Write headings, labels and node names in simple English terms (Process, Function, Location...). Diagrams/drawings/charts appear at 40% size on the right side and float beside notes automatically, so labels must be 1-3 words. Handwritten slide board that follows your speech. BOARD FIRST, VOICE SECOND: always call this tool BEFORE you say a point out loud, so the point is already on the board when you start explaining it. IMPORTANT: if the topic has any structure (anatomy, cycle, steps, parts of a whole, hierarchy, process, data comparison), the diagram/chart block goes in the SAME call as the point it illustrates — never a bare diagram with no point, and never a point with no diagram or chart when one would help. New topic/subtopic: call with a title (topic name) + its first point(s) (+ a diagram/chart block), never a title alone — starts a new slide; then explain it aloud. For each next point: call again with append:true and just that point (same slide, under the heading), then explain it aloud. Add a short 'Example: ...' line (a simple real-life example) for every concept. Short lines. Keep text clean and format key terms with **bold** — do not use raw color markup tags.",
    parameters: {
        type: "OBJECT",
        properties: {
            ...
            blocks: {
                type: "ARRAY",
                description: "Content blocks shown top to bottom. Always include at least one diagram or chart block along with text/notes.",
                items: {
                    type: "OBJECT",
                    properties: {
                        type: { type: "STRING", description: "One of: heading, text, list, steps, flow, note, code, compare, diagram, chart, bars, draw, image, mcq, truefalse, wyr, flashcard." },
                        text: { type: "STRING", description: "For heading, text, note, code." },
                        items: { type: "ARRAY", items: { type: "STRING" }, description: "For list, steps, bars, or chart. For charts/bars use 'Label: 45%' or 'Label: value'." },
                        nodes: { type: "ARRAY", items: { type: "STRING" }, description: "For flow and diagram: the boxes, in order (max 8)." },
                        kind: { type: "STRING", description: "For diagram: cycle (loop/circulation), hub (centre with spokes), or tree (root with branches). For chart: bar, column, pie, donut, or line." },
...
const CONTROL_TOOL = {
    name: "control_iota",
    description: "Control the Iota app itself. Use it ONLY when the user explicitly asks you to change something in the app: 'switch to coach', 'turn the camera on', 'mute my mic', 'start a new chat', 'roast me harder'. Not for teaching content — use show_on_board for that. You must NEVER change your voice.",
    parameters: {
        type: "OBJECT",
        properties: {
...
export class GeminiLiveVoice {
    ...
    async start(systemInstruction, voiceName, opts = {}) {
        ...
        try {
            ...
            // Match on persona/voice/language only, NOT the full instruction text:
            // systemInstruction also carries recentContext()/roastMemory(), which
            // changes on almost every message, so comparing the full string made
            // the prewarmed socket "not match" nearly every time and forced a slow
            // reconnect just to open the avatar screen. opts.reuseKey (passed by
            // the caller) is the stable part; fall back to the old behavior if the
            // caller doesn't pass one.
            const key = opts.reuseKey || `${systemInstruction}|${voiceName}`;
    ...
    prewarm(systemInstruction, voiceName, reuseKey) {
        const apiKey = this.config.get("gemini_api_key");
        if (!apiKey || this.connected || this._opening) return;
        if (this.ws && this.ws.readyState <= WebSocket.OPEN) return;
        const activeVoice = voiceName || this._voice || this.config.get("live_voice_name") || "Kore";
        this._voice = activeVoice;
        this._sys = systemInstruction;
        this._apiKey = apiKey;
        this._closingByUser = false;
        this._wsKey = reuseKey || `${systemInstruction}|${activeVoice}`;
        this._opening = this._openSocket(apiKey, systemInstruction, activeVoice, null)
    ...
    async _reconnect() {
        ...
        this._setState("connecting");
        const reconnectVoice = this._voice || this.config.get("live_voice_name") || "Kore";
        for (let i = 0; i < 4 && !this._closingByUser; i++) {
            await new Promise((r) => setTimeout(r, 300 * (i + 1)));
            if (this._closingByUser) break;
            try {
                await this._openSocket(this._apiKey, this._sys, reconnectVoice, this._resumeHandle);
                this.connected = true;
                this._quietSince = Date.now();
                this._reconnecting = false;
    ...
    _handleServerMessage(msg) {
        ...
        if (serverContent.interrupted) {
            ...
            this._pendingAssistantText = "";
            if (cut && this.onAssistantUtterance) this.onAssistantUtterance(stripBoardMarkup(cut));
        ...
        if (serverContent.outputTranscription && serverContent.outputTranscription.text) {
            ...
            this.onCaption && this.onCaption(stripBoardMarkup(this._pendingAssistantText));
        }
        ...
        if (serverContent.turnComplete) {
            ...
            if (assistantText && this.onAssistantUtterance) this.onAssistantUtterance(stripBoardMarkup(assistantText).trim());
