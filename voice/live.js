...
const BOARD_TOOL = {
    name: "show_on_board",
    description: "Show notes, a diagram, a picture or an interactive quiz on the whiteboard under you while you explain. Call this automatically, unprompted, every time you start explaining something — never wait for the user to ask for a diagram. Keep text short (phrases, not paragraphs) and put each idea in its own block. Write headings, labels and node names in simple English terms (Process, Function, Location...). Diagrams/drawings appear SMALL at the top right and float beside their text automatically, so labels must be 1-3 words. Handwritten slide board that follows your speech. BOARD FIRST, VOICE SECOND: always call this tool BEFORE you say a point out loud, so the point is already on the board when you start explaining it. IMPORTANT: if the topic has any structure (a cycle, steps, parts of a whole, a hierarchy, a process), the diagram block goes in the SAME call as the point it illustrates — never a bare diagram with no point, and never a point with no diagram when one would help. New topic/subtopic: call with a title (topic name) + its first point(s) (+ a diagram block if the topic has structure), never a title alone — starts a new slide; then explain it aloud. For each next point: call again with append:true and just that point (same slide, under the heading), then explain it aloud. Add a short 'Example: ...' line (a simple real-life example) for every concept. Short lines. Colour a key word now and then with {r:x} {b:x} {g:x} {o:x} {p:x} — these tags are for board text only; never say them, never write them in your speech.",
    parameters: {
        type: "OBJECT",
        properties: {
...
export class GeminiLiveVoice {
    ...
    prewarm(systemInstruction, voiceName, reuseKey) {
        ...
        this._closingByUser = false;
        this._wsKey = reuseKey || `${systemInstruction}|${voiceName}`;
