...
const BOARD_TOOL = {
    name: "show_on_board",
    description: "Show notes, a diagram, a picture or an interactive quiz on the whiteboard under you while you explain. Keep text short (phrases, not paragraphs) and put each idea in its own block. Write headings, labels and node names in simple English terms (Process, Function, Location...). Diagrams/drawings appear SMALL at the top right, so labels must be 1-3 words. Handwritten digital slide board: every call WITHOUT append makes a NEW slide — ONE topic per slide, always with a title (the topic heading; it shows centred for a moment, then moves to the top with the content under it); a subtopic or next topic = a new slide with its own title; append adds to the current slide. Colour key words sparingly with {r:word} {b:word} {g:word} {o:word} {p:word}.",
    parameters: {
        type: "OBJECT",
        properties: {
...
const CONTROL_TOOL = {
    ...
    parameters: {
        type: "OBJECT",
        properties: {
            action: { type: "STRING", description: "One of: set_persona (value: friend|teacher|mother|girlfriend|boyfriend|coach), set_voice (value: a Gemini voice name such as Kore, Puck, Charon, Zephyr, Aoede), set_language (value: hindi|bengali|english), new_chat, camera (value: on|off|flip), mic (value: mute|unmute), board (value: clear|expand|collapse), roast_level (value: off|mild|medium|savage), open (value: settings|history), go_home." },
