...
export const BOARD_TOOL = {
    name: "show_on_board",
    description: "Show notes, diagrams, charts, pictures or interactive quiz cards on the whiteboard under you while you explain. Call this automatically, unprompted, every time you start explaining something — never wait for the user to ask. CRITICAL RULE: The left side of the whiteboard must NEVER be left empty! The visual figure (diagram/chart/image/draw) floats automatically on the right side at 40% width. Therefore, in the EXACT SAME tool call, you MUST provide 2 to 4 structured point-by-point explanation bullet items (using a 'list' or 'steps' block) PLUS a real-life example (using a 'note' block starting with 'Example: ...') to fill the left side! NEVER call this tool with only an image, diagram, or chart without point-by-point bullet items and an example beside it. Write headings, labels and node names in simple English terms (Process, Function, Location...). BOARD FIRST, VOICE SECOND: always call this tool BEFORE you say the points out loud, so the points and diagram are already on the board when you speak. When speaking, explain point-by-point with a clear real-life example, and playfully tease the user with witty double-meaning humor and savage roasts. Short clean lines. Use **bold** for key terms — do not use raw color tags.",
    parameters: {
        type: "OBJECT",
        properties: {
            ...
            blocks: {
                type: "ARRAY",
                description: "Content blocks shown on the board. MUST ALWAYS include: (1) A 'list' or 'steps' block with 2-4 point-by-point bullet items on the left, (2) A 'note' block with 'Example: ...' on the left, and (3) At least one diagram, chart, or image block floated at 40% on the right. The left side must never be left empty!",
                items: {
                    type: "OBJECT",
                    properties: {
