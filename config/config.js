...
export class NoaConfig {
    ...
    async init() {
        try {
            ...
            console.warn("[NoaConfig] Failed to load settings from IndexedDB, using defaults.", err);
        }
        try {
            const sessions = await idb.getAll("sessions");
            this._sessions = (sessions || []).sort((a, b) => b.ts - a.ts).slice(0, SESSION_LIMIT);
        } catch (err) {
            console.warn("[NoaConfig] Failed to load sessions from IndexedDB.", err);
        }
        this._ready = true;
        return this;
    }
    ...
    save() {
        idb.put("kv", { id: SETTINGS_ROW_ID, value: { ...this._data } }).catch((err) => {
            console.error("[NoaConfig] Failed to save settings to IndexedDB.", err);
        });
        return true;
    }
    ...
    // Whether Noa's brain (Gemini) is configured — the only hard
    ...
    _persistSession(session) {
        idb.put("sessions", session).catch((err) => {
            console.warn("[NoaConfig] Failed to save session to IndexedDB.", err);
        });
    }
