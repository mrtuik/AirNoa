...
const VIDEO_SRC = {
    idle: "./assets/iota-idle.mp4",
    speaking: "./assets/iota-speaking.mp4"
};

export class IotaModel {
    /**
    ...
    mount() {
        if (this._wrapper) return this; // already mounted/preloaded
        this.container.innerHTML = "";

        const wrapper = document.createElement("div");
        wrapper.className = "iota-model-wrapper iota-state-idle";

        const glow = document.createElement("div");
        glow.className = "iota-model-glow";
        wrapper.appendChild(glow);

        const frameHolder = document.createElement("div");
        frameHolder.className = "iota-model-frame-holder";

        const videoIdle = this._createVideo("iota-idle", VIDEO_SRC.idle);
        const videoSpeaking = this._createVideo("iota-speaking", VIDEO_SRC.speaking);

        // Idle clip visible by default (matches the initial "idle" state);
        // speaking clip stays hidden until we actually enter "speaking".
        // Both clips stay in the DOM; we cross-swap opacity (no display:none
        // toggling), so the speaking clip is already decoded when needed.
        videoIdle.style.opacity = "1";
        videoSpeaking.style.opacity = "0";
        ...
        // Preload: warm the speaking clip (play a beat, park at frame 0) so the
        // first switch to speaking is instant.
        const warm = () => {
            this._safePlay(videoSpeaking);
            setTimeout(() => {
                if (this.state !== "speaking") { videoSpeaking.pause(); try { videoSpeaking.currentTime = 0; } catch (e) { /* ignore */ } }
            }, 150);
        };
        if (videoSpeaking.readyState >= 3) warm(); else videoSpeaking.addEventListener("canplaythrough", warm, { once: true });
    ...
    _createVideo(id, src) {
        ...
        video.id = id;
        video.className = "iota-model-video";
        video.src = src;
        video.muted = true;
        video.autoplay = true;
        video.preload = "auto";
        ...
        video.style.objectPosition = "50% 0%"; // keep the head; the lower body fades into the whiteboard (see iota.html .stage-blur)
    ...
    setState(state) {
        if (!VALID_STATES.includes(state)) {
            console.warn(`[IotaModel] Ignoring unknown state: ${state}`);
            return;
        }
        this.state = state;
        if (!this._wrapper) return;

        VALID_STATES.forEach((s) => this._wrapper.classList.remove(`iota-state-${s}`));
        this._wrapper.classList.add(`iota-state-${state}`);
    ...
    _applyStateToVideo(state) {
        ...
        if (state === "speaking") {
            this._videoIdle.style.opacity = "0";
            this._videoSpeaking.style.opacity = "1";
            ...
            this._videoSpeaking.style.opacity = "0";
            this._videoSpeaking.pause();

            this._videoIdle.style.opacity = "1";
