# Noa (AirC) — Project Memory

Single-file-flavored web app (`AirC.html` + ES modules) — a voice-first AI
companion avatar, now branded "Noa", that listens, replies via Gemini, and
...
## File map
- `AirC.html` — UI (chat/voice screens, Settings sheet, persona picker,
...
- `db/idb.js` — tiny promise-based raw IndexedDB wrapper (two object
  ...
  network calls.
- `config/config.js` — `NoaConfig` class: owns ALL persistent settings and
...
- `brain/brain.js` — `NoaBrain`: Gemini **text** generation
  (`gemini-2.0-flash`, REST `generateContent`) for the typed chat only.
...
- `model/model.js` — `AirCModel`: avatar state machine
...
- Model: `gemini-2.5-flash-native-audio-preview-09-2025` — Google's current
  native-audio-to-audio Live API preview model as of this writing. Google
  periodically renames/retires "-preview-" models; if this one 404s, swap
  in whatever the current native-audio Live model is per the Gemini API
  docs (just the `LIVE_MODEL` constant).
...
- Mic capture uses `ScriptProcessorNode` (deprecated but universally
  ...
  to the speakers; manual nearest-neighbor downsample to 16kHz since
...
- The silence-timeout / low-effort "roast" nudge mechanic from the old
  ...
  old "sassy AirClia" personality, which doesn't fit personas like
  Teacher/Mother.
